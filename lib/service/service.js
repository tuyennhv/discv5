...
import { createPingMessage, createFindNodeMessage, createNodesMessage, MessageType, requestMatchesResponse, createPongMessage, createTalkRequestMessage, createTalkResponseMessage, } from "../message/index.js";
...
export class Discv5 extends EventEmitter {
    ...
    static create({ enr, peerId, multiaddr, config = {}, metrics, transport }) {
        ...
        const sessionService = new SessionService(fullConfig, decodedEnr, createKeypairFromPeerId(peerId), transport ?? new UDPTransportService(multiaddr, decodedEnr.nodeId));
     ...
     */
    sendRpcResponse(nodeAddr, response) {
    ...
    handleWhoAreYouRequest = (nodeAddr, nonce) => {
     ...
     */
    handleRpcRequest = (nodeAddr, request) => {
    ...
    handlePing(nodeAddr, message) {
        ...
        try {
            ...
            this.sessionService.sendResponse(nodeAddr, createPongMessage(message.id, this.enr.seq, srcOpts.host, srcOpts.port));
     ...
     */
    handleRpcResponse = (nodeAddr, response) => {
