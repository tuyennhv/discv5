...
class Discv5 extends events_1.EventEmitter {
    ...
    constructor(config, sessionService, metrics) {
        ...
        this.started = false;
        // process kad updates
        this.onPendingEviction = async (enr) => {
        ...
        this.onEstablished = async (enr, direction) => {
            ...
            await this.connectionUpdated(nodeId, { type: _1.ConnectionStatusType.Connected, enr, direction });
        };
        this.handleWhoAreYouRequest = async (nodeAddr, nonce) => {
         ...
         */
        this.handleRpcRequest = async (nodeAddr, request) => {
         ...
         */
        this.handleRpcResponse = async (nodeAddr, response) => {
         ...
         */
        this.rpcFailure = async (rpcId, error) => {
            ...
            return this.connectionUpdated(nodeId, { type: _1.ConnectionStatusType.Disconnected });
        };
        this.config = config;
     ...
     */
    static create({ enr, peerId, multiaddr, config = {}, metrics, transport }) {
        const fullConfig = { ...config_1.defaultConfig, ...config };
        const decodedEnr = typeof enr === "string" ? enr_1.ENR.decodeTxt(enr) : enr;
        const sessionService = new session_1.SessionService(fullConfig, decodedEnr, keypair_1.createKeypairFromPeerId(peerId), transport ?? new transport_1.UDPTransportService(multiaddr, decodedEnr.nodeId));
     ...
     */
    async sendTalkReq(remote, payload, protocol) {
        return await new Promise((resolve, reject) => {
            this.sendRpcRequest({
                contact: nodeInfo_1.createNodeContact(remote),
     ...
     */
    async sendTalkResp(remote, requestId, payload) {
        const msg = message_1.createTalkResponseMessage(requestId, payload);
        const nodeAddr = nodeInfo_1.getNodeAddress(nodeInfo_1.createNodeContact(remote));
     ...
     */
    async sendPing(enr) {
        return this.sendRpcRequest({ contact: nodeInfo_1.createNodeContact(enr), request: message_1.createPingMessage(this.enr.seq) });
    }
    /**
     * Ping all peers connected in the routing table
     */
    async pingConnectedPeers() {
     ...
     */
    async requestEnr(contact, callback) {
        return this.sendRpcRequest({ request: message_1.createFindNodeMessage([0]), contact, callback });
    }
    /**
     * Constructs and sends a request to the session service given a target and lookup peer
     */
    async sendLookup(lookupId, peer, request) {
     ...
     */
    async sendRpcRequest(activeRequest) {
     ...
     */
    async sendRpcResponse(nodeAddr, response) {
        log("Sending %s to node: %o", message_1.MessageType[response.type], nodeAddr);
        try {
            await this.sessionService.sendResponse(nodeAddr, response);
            this.metrics?.sentMessageCount.inc({ type: message_1.MessageType[response.type] });
        }
        catch (e) {
            log("Error sending RPC to node: %o, :Error: %s", nodeAddr, e.message);
        }
     ...
     */
    async connectionUpdated(nodeId, newStatus) {
            ...
            case _1.ConnectionStatusType.Connected: {
                ...
                switch (this.kbuckets.insertOrUpdate(newStatus.enr, kademlia_1.EntryStatus.Connected)) {
                    case kademlia_1.InsertResult.Inserted: {
                        ...
                        this.connectedPeers.set(nodeId, setInterval(async () => {
    ...
    async handlePing(nodeAddr, message) {
        ...
        if (entry) {
            if (entry.value.seq < message.enrSeq) {
                await this.requestEnr(nodeInfo_1.createNodeContact(entry.value));
            }
        }
        ...
        try {
            ...
            await this.sessionService.sendResponse(nodeAddr, message_1.createPongMessage(message.id, this.enr.seq, srcOpts.host, srcOpts.port));
     ...
     */
    async handleFindNode(nodeAddr, message) {
        ...
        if (nodes.length === 0) {
            ...
            try {
                ...
                await this.sessionService.sendResponse(nodeAddr, message_1.createNodesMessage(id, 0, nodes));
        ...
        for (let i = 0; i < nodes.length; i += nodesPerPacket) {
            ...
            try {
                await this.sessionService.sendResponse(nodeAddr, message_1.createNodesMessage(id, total, _nodes));
                this.metrics?.sentMessageCount.inc({ type: message_1.MessageType[message_1.MessageType.NODES] });
            }
            catch (e) {
                log("Failed to send a NODES response. Error: %s", e.message);
    ...
    async handlePong(nodeAddr, activeRequest, message) {
        ...
        if (enr) {
            ...
            await this.connectionUpdated(nodeAddr.nodeId, { type: _1.ConnectionStatusType.PongReceived, enr });
        }
    }
