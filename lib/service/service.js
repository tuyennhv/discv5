...
import { UDPTransportService } from "../transport/index.js";
...
import { ENR, MAX_RECORD_SIZE, createNodeId } from "../enr/index.js";
import { createKeypairFromPeerId, createPeerIdFromKeypair } from "../keypair/index.js";
import { EntryStatus, InsertResult, KademliaRoutingTable, log2Distance, Lookup, UpdateResult, } from "../kademlia/index.js";
import { createPingMessage, createFindNodeMessage, createNodesMessage, MessageType, requestMatchesResponse, createTalkRequestMessage, createTalkResponseMessage, } from "../message/index.js";
import { AddrVotes } from "./addrVotes.js";
import { toBuffer } from "../util/index.js";
import { defaultConfig } from "../config/index.js";
import { createNodeContact, getNodeAddress, getNodeId } from "../session/nodeInfo.js";
import { ConnectionStatusType, } from "./types.js";
import { RateLimiter } from "../rateLimit/index.js";
import { getSocketAddressOnENR, multiaddrFromSocketAddress, isEqualSocketAddress, multiaddrToSocketAddress, setSocketAddressOnENR, } from "../util/ip.js";
...
export class Discv5 extends EventEmitter {
    ...
    static create(opts) {
        ...
        const sessionService = new SessionService(fullConfig, decodedEnr, createKeypairFromPeerId(peerId), transport ?? new UDPTransportService(multiaddr, decodedEnr.nodeId, rateLimiter));
    ...
    async sendTalkResp(remote, requestId, payload) {
        ...
        await this.sendRpcResponse(remote, msg);
    }
    /**
     ...
     */
    async sendPing(nodeAddr) {
        return this.sendRpcRequest({ contact: createNodeContact(nodeAddr), request: createPingMessage(this.enr.seq) });
    }
    /**
    ...
    // process kad updates
    onPendingEviction = async (enr) => {
        return this.sendPing(enr);
    };
    onAppliedEviction = (inserted, evicted) => {
        this.emit("enrAdded", inserted, evicted);
    };
    // process events from the session service
    onEstablished = (nodeAddr, enr, direction, verified) => {
    ...
    handleWhoAreYouRequest = async (nodeAddr, nonce) => {
     ...
     */
    handleRpcRequest = async (nodeAddr, request) => {
    ...
    async handlePing(nodeAddr, message) {
        ...
        try {
            await this.sessionService.sendResponse(nodeAddr, pongMessage);
    ...
    async handleFindNode(nodeAddr, message) {
        ...
        if (nodes.length === 0) {
            ...
            try {
                ...
                await this.sessionService.sendResponse(nodeAddr, createNodesMessage(id, 1, nodes));
    ...
    handleTalkReq = (nodeAddr, message) => {
     ...
     */
    handleRpcResponse = async (nodeAddr, response) => {
    ...
    handleTalkResp = (nodeAddr, activeRequest, message) => {
