...
import { AUTHDATA_SIZE_SIZE, EPH_KEY_SIZE_SIZE, ERR_INVALID_FLAG, ERR_INVALID_PROTOCOL_ID, ERR_INVALID_VERSION, ERR_INVALID_AUTHDATA_SIZE, ERR_TOO_LARGE, ERR_TOO_SMALL, FLAG_SIZE, MASKING_IV_SIZE, MASKING_KEY_SIZE, MAX_PACKET_SIZE, MESSAGE_AUTHDATA_SIZE, MIN_PACKET_SIZE, NONCE_SIZE, PROTOCOL_SIZE, SIG_SIZE_SIZE, STATIC_HEADER_SIZE, VERSION_SIZE, WHOAREYOU_AUTHDATA_SIZE, ID_NONCE_SIZE, MIN_HANDSHAKE_AUTHDATA_SIZE, } from "./constants.js";
import { PacketType } from "./types.js";
export function encodePacket(destId, packet) {
    return Buffer.concat([packet.maskingIv, encodeHeader(destId, packet.maskingIv, packet.header), packet.message]);
}
export function encodeHeader(destId, maskingIv, header) {
...
export function decodePacket(srcId, data) {
 ...
 */
export function decodeHeader(srcId, maskingIv, data) {
    ...
    const flag = bufferToNumber(staticHeaderBuf.slice(PROTOCOL_SIZE + VERSION_SIZE, PROTOCOL_SIZE + VERSION_SIZE + FLAG_SIZE), FLAG_SIZE);
    if (PacketType[flag] == null) {
        throw errcode(new Error(`Invalid flag: ${flag}`), ERR_INVALID_FLAG);
    }
    const nonce = staticHeaderBuf.slice(PROTOCOL_SIZE + VERSION_SIZE + FLAG_SIZE, PROTOCOL_SIZE + VERSION_SIZE + FLAG_SIZE + NONCE_SIZE);
    const authdataSize = bufferToNumber(staticHeaderBuf.slice(PROTOCOL_SIZE + VERSION_SIZE + FLAG_SIZE + NONCE_SIZE), AUTHDATA_SIZE_SIZE);
...
// authdata
export function encodeWhoAreYouAuthdata(authdata) {
    return Buffer.concat([authdata.idNonce, toBufferBE(authdata.enrSeq, 8)]);
}
export function encodeMessageAuthdata(authdata) {
    return fromHex(authdata.srcId);
}
export function encodeHandshakeAuthdata(authdata) {
...
export function decodeWhoAreYouAuthdata(data) {
...
export function decodeMessageAuthdata(data) {
...
export function decodeHandshakeAuthdata(data) {
 ...
 */
export function encodeChallengeData(maskingIv, header) {
    return Buffer.concat([
        maskingIv,
