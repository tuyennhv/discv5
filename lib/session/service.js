...
import { PacketType, decodeHandshakeAuthdata, decodeMessageAuthdata, decodeWhoAreYouAuthdata, encodeChallengeData, createHeader, encodeMessageAuthdata, createRandomPacket, createWhoAreYouPacket, } from "../packet/index.js";
import { ERR_INVALID_SIG, Session } from "./session.js";
import { encode, decode, MessageType, createFindNodeMessage, isRequestType, } from "../message/index.js";
import { RequestErrorType, ConnectionDirection, } from "./types.js";
import { getNodeAddress, INodeContactType, nodeAddressToString } from "./nodeInfo.js";
...
export class SessionService extends EventEmitter {
    ...
    constructor(config, enr, keypair, transport) {
        ...
        this.activeRequests = new TimeoutMap(config.requestTimeout, (k, v) => this.handleRequestTimeout(getNodeAddress(v.contact), v));
     ...
     */
    async sendRequest(contact, request) {
     ...
     */
    async sendResponse(nodeAddr, response) {
        const nodeAddrStr = nodeAddressToString(nodeAddr);
     ...
     */
    async sendChallenge(nodeAddr, nonce, remoteEnr) {
        const nodeAddrStr = nodeAddressToString(nodeAddr);
    ...
    processInboundPacket = async (src, packet) => {
    ...
    async handleChallenge(src, packet) {
        ...
        const [authPacket, session] = Session.encryptWithHeader(requestCall.contact, this.keypair, this.enr.nodeId, updatedEnr, encodeChallengeData(packet.maskingIv, packet.header), encode(requestCall.request));
    ...
    async handleHandshake(src, packet) {
        ...
        try {
            const [session, enr] = Session.establishFromChallenge(this.keypair, this.enr.nodeId, nodeAddr.nodeId, challenge, authdata.idSignature, authdata.ephPubkey, authdata.record);
    ...
    async handleMessage(src, packet) {
        let authdata;
        try {
        ...
        let encodedMessage;
        try {
            encodedMessage = session.decryptMessage(packet.header.nonce, packet.message, packet.messageAd || encodeChallengeData(packet.maskingIv, packet.header));
        }
        catch (e) {
        ...
        if (isRequestType(message.type)) {
            ...
            await this.handleResponse(nodeAddr, message);
        }
    }
    /**
     ...
     */
    async handleResponse(nodeAddr, response) {
        const nodeAddrStr = nodeAddressToString(nodeAddr);
    ...
    async handleRequestTimeout(nodeAddr, requestCall) {
    ...
    async sendNextRequest(nodeAddr) {
        const nodeAddrStr = nodeAddressToString(nodeAddr);
